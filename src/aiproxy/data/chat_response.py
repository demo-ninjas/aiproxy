...
class ChatResponse: 
    ...
    failed:bool = False
    """Whether the response failed to be generated due to an error."""
    error:str|None = None
    ...
    def to_api_response(self) -> dict:
        ...
        if self.filtered: out["filtered"] = True
        if self.filter_reason is not None: out["filter-reason"] = self.filter_reason
        if self.failed: out["failed"] = True
